...
public class Main {
	...
	static HashMap<String, Circle> supportedCircles = new HashMap<String, Circle>();
		...
		try {
			...
			for (File plugin_jar : plugins) {
				try {
					...
					Class<Circle> cls = (Class<Circle>) cl.loadClass("plugins.PluginCircle");
	...
	private static void drawCircle() {
		...
		if (user_pref_circle==null || radius<=0) {
			System.out.println("Unsuppoorted Circle Options");
			return;
		}
